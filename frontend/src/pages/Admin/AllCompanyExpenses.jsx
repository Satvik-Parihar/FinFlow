...
export default function AllCompanyExpenses() {
    ...
    useEffect(() => {
        const fetchAll = async () => {
            try {
                const response = await api.get('/admin/all-expenses');
                setExpenses(response.data);
            } catch (err) {
                setError(err.response?.data?.message || "Failed to fetch company expenses.");
            }
            finally {
                setIsLoading(false);
            }
    ...
    if (isLoading) return <p className="text-center p-8">Loading company expenses...</p>;
    if (error) return <p className="text-center text-red-600 p-8">{error}</p>;

    return (
        <div className="bg-white p-4 sm:p-8 rounded-2xl shadow-lg">
            <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-6">All Company Expenses (Read-Only)</h2>
            {expenses.length === 0 ? <p className="text-center text-gray-500 py-4">No expenses have been submitted in the company yet.</p> :
